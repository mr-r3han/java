...
public class HDFCBank extends AllBank {
    private static HDFCBank HDFCObject = null;  

    long balance = 25000;

    public void HDFCOperation(){
        Scanner in = new Scanner(System.in);
        
        while(true){
            if(blockCount == 3) break;
            System.out.println(ANSI_YELLOW+"===================Welcome back to HDFC Bank==================="+ANSI_RESET);
            ...
            try{
                ...
                if(res==1){
                    ...
                    System.out.println(ANSI_YELLOW+"=====================Thanks for using HDFC Bank================================"+ANSI_RESET);
                    break;
                }else{
    ...
    public void bankBalance(){
        while(true){
            ...
            try{
                ...
                System.out.print("Please enter HDFC Bank UPI pin to check Balance : ");
                int axisPinUser = in.nextInt();
                PhoneMain pm = PhoneMain.PhonePayObject();
                if(axisPinUser != pm.getHDFCPin()){
            ...
            try{
                ...
                try{
                    if(count<10 || count>10) throw new numberCheckException();
                        ...
                        System.out.println(ANSI_YELLOW+"================================================="+ANSI_RESET);
                        System.out.println("The mobile number "+number+" is verifed by HDFC bank.");
                        while(true){
                            try{
                                ...
                                if(amountTrans<=0){ 
                                    ...
                                    while(true){
                                        ...
                                        try{
                                            System.out.print("Enter the HDFC Bank UPI pin : ");
                                            int pin = in.nextInt();
                                            PhoneMain pm = PhoneMain.PhonePayObject();
                                            if(pin!=pm.getHDFCPin()){ 
                                                blockCount++;
                                                System.out.println("Invalid pin...");
                                            }
                                            else{
                                                HDFCBank ab = HDFCBank.getHDFCObject();
                                                try{
                                                    if(amountTrans>ab.balance) throw new balanceCheckException();
                                                        ...
                                                        String s = amountTrans + " is transfered to number " + number + ".";
    ...
    public static HDFCBank getHDFCObject(){
        if(HDFCObject==null){
            HDFCObject = new HDFCBank();
        }
        return HDFCObject;
    }
}
